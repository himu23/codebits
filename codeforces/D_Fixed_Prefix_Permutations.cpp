...
void solve() {
    ...
    // for(int i=0;i<n;i++)
    sort(temp.begin(),temp.end()); //keeps the inside elements intact
    for(int i=0;i<n;i++){
        // auto it=lower_bound(temp.begin(),temp.end(),p[i]);
        // if(it!=temp.end()){
        ...
        if(idx==n){
            ...
            for(int j=0;j<m;j++){
                if(p[i][j]!=temp[idx-1][j]){
                    // cur=max(cur,j);break;
                    curl=j;break;
                }
            }
            if(curl==-1 || curr==-1) curans=cur;
            else{
                curans=max(curl,curr);
            }
            // curans=cur;
        }
        cout<<curans<<" ";
