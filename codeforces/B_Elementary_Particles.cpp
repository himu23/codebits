...
void solve() {
    ...
    for(auto x:um){
        ...
        vector<ll> vec=x.se;
        // ll m=lower_bound(vec.begin(),vec.end(),n/2)-vec.begin();
        // if(m!=vec.size()-1) ans=max(ans,vec[m]+n-vec[m+1]);
        // if(m!=0) ans=max(ans,vec[m-1]+n-vec[m]);
        for(int i=1;i<vec.size();i++){
            ans=max(ans,n-(vec[i]-vec[i-1]));
        }
    }
    cout<<ans<<endl;
}
