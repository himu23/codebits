...
void solve() {
    ll x,p,y,q; cin>>x>>p>>y>>q;
    int t1=to_string(x).length();
    int t2=to_string(y).length();
    if(t1+p>t2+q) {cout<<">"<<endl;return;}
    else if(t1+p<t2+q) {cout<<"<"<<endl;return;}
    ld a=(ld)x,b=(ld)y;
    if(p>q) b/=binpow(10,p-q);
    else a/=binpow(10,q-p);
    if(a>b) {cout<<">"<<endl;return;}
