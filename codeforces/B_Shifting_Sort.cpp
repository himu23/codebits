...
void solve() {
    ...
    //     int val=vec[t];
    //     int v1=a[t];
    //     for(int j=t;j>i;j--) {vec[j]=vec[j-1];a[j]=a[j-1];}
    //     vec[i]=val;
    //     a[i]=v1;
    // }
    // cout<<ans.size()<<endl;
    // for(int i=0;i<ans.size();i++){
    //     cout<<ans[i].fi+1<<" "<<ans[i].se+1<<" "<<1<<endl;
    // }
    ...
    for(int i=0;i<n;i++){
        ...
        int idx;
        for(int j=i;j<n;j++){
            if(a[j]==cur){idx=j;break;}
        }
        if(i==idx) continue;
        int d=idx-i;
        ans.pb({{min(i,idx),max(i,idx)},d});
        int val=a[idx];
    ...
    cout<<ans.size()<<endl;
    for(int i=0;i<ans.size();i++){
        cout<<ans[i].fi.fi+1<<" "<<ans[i].fi.se+1<<" "<<ans[i].se<<endl;
    }
}
